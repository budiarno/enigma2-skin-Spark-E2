# 2016.08.13 15:34:20 WIT
#Embedded file name: /usr/lib/enigma2/python/Components/Converter/CIInfo.py
...
class CIInfo(Poll, Converter, object):
    CAID = 0
    PID = 1
    PROV = 2
    ALL = 3
        ...
        if service:
            ...
            if info:
                if info.getInfoObject(iServiceInformation.sCAIDs):
                    ...
                    if ecm_info:
                        ...
                        if self.type == self.MYEMU:
                            if source == 'emu':
                                ...
                                textvalue = '%s - %s, Hops %s, Time %s' % (protocol,
                                 reader,
                                 hops,
                                 ecm_time.replace('msec', 'ms'))
                            else:
                    ...
                    elif self.type == self.ALL or self.type == self.SHORT or self.type == self.MYEMU or self.type == self.FORMAT and self.sfmt.count('%') > 3:
                        textvalue = 'Encrypted'
                elif self.type == self.ALL or self.type == self.SHORT or self.type == self.MYEMU or self.type == self.FORMAT and self.sfmt.count('%') > 3:
