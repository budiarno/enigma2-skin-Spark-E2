...
class QuickEcmInfo2(Poll, Converter, object):
	...
	def getText(self):
		...
		if self.type == self.vtype:
			...
			try:
				...
				if audio:
					...
					audioTrackCodec = ""
				return "%d kbit/s" % (str(xres), str(yres) + mode, self.getServiceInfoString(info, iServiceInformation.sFrameRate, lambda x: "%d" % ((x+500)/1000)), ("MPEG2", "MPEG4", "MPEG1", "MPEG4-II", "VC1", "VC1-SM", "")[info.getInfo(iServiceInformation.sVideoType)], self.video, audioTrackCodec, self.audio)
		...
		elif self.type == self.vres:
			try:
				yres = info.getInfo(iServiceInformation.sVideoHeight)
				mode = ("i", "p", "")[info.getInfo(iServiceInformation.sProgressive)]
				xres = info.getInfo(iServiceInformation.sVideoWidth)
				return "%sx%s(%s)" % (str(xres), str(yres) + mode, self.getServiceInfoString(info, iServiceInformation.sFrameRate, lambda x: "%d" % ((x+500)/1000)))
			except: 
				return " "
...
###############################################################################
			...
			file_input='/tmp/ecm.info'
			cws=subprocess.Popen(['grep','^cw',file_input], stdout= subprocess.PIPE)
			if os.path.exists(file_input):
				return cws.stdout.read()
			else:
				return "No info available. Possibly it's FTA or no cam is supported."

		elif self.type == self.cpu:
#			cpuinfo = os.system('top|grep "CPU"|awk 'NR==1{print $1 $2}' ')
			cpus=subprocess.call(['grep','CPU',cpuinfo], stdout= subprocess.PIPE)
			return cpus.stdout.read()
